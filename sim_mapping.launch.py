...
def generate_launch_description():
    ...
    default_map_db_out_arg = '../data/map/gazebo480-' + time.strftime("%Y%m%d-%H%M") + '.msg'

    map_db_out_arg = DeclareLaunchArgument(
        'map_db_out',
        default_value=default_map_db_out_arg,
        description='Path to output map db file')
    ...
    stella_ros = Node(
        ...
        arguments=["-v","./orb_vocab.fbow",
                   "-c","./gazebo_480p.yaml",
                   "--map-db-out",LaunchConfiguration('map_db_out')
